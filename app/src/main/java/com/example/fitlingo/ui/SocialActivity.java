...
import com.example.fitlingo.data.FriendRepository;
...
public class SocialActivity extends BaseNavActivity {
  ...
  private void loadFriends() {
    ...
    friends.listFriendIds().addOnSuccessListener(ids -> {
      ...
      java.util.List<String> friendUids = new java.util.ArrayList<>();
      for (DocumentSnapshot d : ids) friendUids.add(String.valueOf(d.get("friendId")));
      friendUids.add(com.example.fitlingo.data.FirebaseManager.auth().getCurrentUser().getUid());
      com.example.fitlingo.data.FirebaseManager.db().collection("users").whereIn("uid", friendUids).get().addOnSuccessListener(rs -> {
        if (rs.isEmpty()) {
          final int[] pending = {friendUids.size()};
          for (String uid : friendUids) {
            com.example.fitlingo.data.FirebaseManager.db().collection("users").document(uid).get().addOnSuccessListener(doc -> {
