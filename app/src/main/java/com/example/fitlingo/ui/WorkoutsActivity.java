...
import com.example.fitlingo.data.SocialRepository;
import com.example.fitlingo.data.WorkoutRepository;
...
public class WorkoutsActivity extends BaseNavActivity {
  ...
  private void loadWorkoutsForSelectedDay() {
    String iso = com.example.fitlingo.util.DateUtils.toIso(selected.getTime());
    com.google.firebase.firestore.Query q = "Todos".equals(filterType) ? repo.listByDate(iso) : repo.listByDateAndType(iso, filterType);
    q.get().addOnSuccessListener(rs -> {
  ...
  public void onSaveWorkout(View v) {
    ...
    try {
      java.util.Date parsed = com.example.fitlingo.util.DateUtils.parseUi(date.getText().toString());
      w.date = com.example.fitlingo.util.DateUtils.toIso(parsed);
  ...
  private void setupSpinners() {
    ...
    android.widget.Spinner filter = findViewById(R.id.filter_type);
    java.util.List<String> ft = new java.util.ArrayList<>();
    ft.add("Todos");
    for (int i = 0; i < types.getCount(); i++) ft.add(String.valueOf(types.getItem(i)));
    android.widget.ArrayAdapter<String> filterAdapter = new android.widget.ArrayAdapter<>(this, android.R.layout.simple_spinner_item, ft);
    filterAdapter.setDropDownViewResource(android.R.layout.simple_spinner_dropdown_item);
    filter.setAdapter(filterAdapter);
    filter.setOnItemSelectedListener(new android.widget.AdapterView.OnItemSelectedListener() {
      @Override public void onItemSelected(android.widget.AdapterView<?> parent, android.view.View view, int position, long id) { filterType = ft.get(position); loadWorkoutsForSelectedDay(); }
  ...
  private void setupDatePickers() {
    ...
    start.setOnClickListener(v -> pickDate(d -> start.setText(com.example.fitlingo.util.DateUtils.toUi(d))));
    end.setOnClickListener(v -> pickDate(d -> {
      start.setError(null);
      java.util.Date s = null; try { s = com.example.fitlingo.util.DateUtils.parseUi(start.getText().toString()); } catch (Exception ignored) {}
      if (s != null && d.before(s)) { end.setError("Debe ser posterior a inicio"); } else { end.setError(null); end.setText(com.example.fitlingo.util.DateUtils.toUi(d)); }
    }));
    date.setOnClickListener(v -> pickDate(d -> date.setText(com.example.fitlingo.util.DateUtils.toUi(d))));
  }
  ...
  private void updateSelectedDateLabel() {
    ((android.widget.TextView)findViewById(R.id.selected_date_label)).setText(com.example.fitlingo.util.DateUtils.toUi(selected.getTime()));
  }
